      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
          {data.map((row, i) => {
            ...
            const itemId =
              rawItem?.canonical_id || rawItem?.id || `coverage-${i}`;
            return (
                  ...
                  <TooltipTrigger asChild>
                    <tr
                      ...
                      onClick={() =>
                        onItemClick?.(
                          "effective_coverage",
                          row.canonicalId || rawItem?.id || `coverage-${i}`,
                        )
                      }
                    >
                      <td className="px-3 py-3 align-top w-[220px] min-w-[180px]">
                        <div className="flex items-center gap-2">
                          ...
                          <FileText className="size-3 text-zinc-400 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
                        </div>
                        {row.type && (
                          <Badge
                            variant="outline"
                            className="mt-1 text-[9px] px-1.5 py-0 h-4 text-zinc-500 border-zinc-200 dark:border-zinc-700"
                          >
                            {row.type}
                          </Badge>
                        )}
                      ...
                      <td className="px-3 py-3 align-top text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
                        {row.description ||
                          "Provides coverage as defined in the policy."}
                      </td>
                    </tr>
                  </TooltipTrigger>
...
export function ExclusionTable({
  ...
  const data = items.map(
    isEntity ? mapEntityToExclusion : mapSectionToExclusion,
  );
  return (
    <div className="overflow-x-auto rounded-md border border-zinc-200 dark:border-zinc-800">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
          {data.map((row, i) => {
            const rawItem = items[i];
            const itemId = row.canonicalId || rawItem?.id || `exclusion-${i}`;
            return (
              <TooltipProvider key={i}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <tr
                      className="group hover:bg-zinc-50/30 dark:hover:bg-zinc-900/10 cursor-pointer"
                      onClick={() =>
                        onItemClick?.("effective_exclusion", itemId)
                      }
                    >
                      <td className="px-3 py-3 align-top w-[220px] min-w-[180px]">
                        <div className="flex items-center gap-2">
                        ...
                        {row.severity && (
                          <Badge
                            variant="outline"
                            className={cn(
                              "mt-1 text-[9px] px-1.5 py-0 h-4 uppercase tracking-tighter",
                      ...
                      <td className="px-3 py-3 align-top text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
                        {row.explanation ||
                          "This insurance does not apply to claims related to this exclusion."}
                      </td>
                    </tr>
                  </TooltipTrigger>
                  <TooltipContent side="left" className="text-[10px] py-1 px-2">
...
export function ConditionTable({
  ...
  const data = items.map(
    isEntity ? mapEntityToCondition : mapSectionToCondition,
  );
  return (
    <div className="overflow-x-auto rounded-md border border-zinc-200 dark:border-zinc-800">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
          {data.map((row, i) => {
            const rawItem = items[i];
            const itemId =
              rawItem?.canonical_id || rawItem?.id || `condition-${i}`;
            return (
              <TooltipProvider key={i}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <tr
                      ...
                      onClick={() =>
                        onItemClick?.(
                          "condition",
                          row.canonicalId || rawItem?.id || `condition-${i}`,
                        )
                      }
                    >
                      <td className="px-3 py-3 align-top w-[220px] min-w-[180px]">
                      ...
                      <td className="px-3 py-3 align-top text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
                        {row.requirement ||
                          "Policy condition as defined in the coverage form."}
                      </td>
                    </tr>
                  </TooltipTrigger>
                  <TooltipContent side="left" className="text-[10px] py-1 px-2">
...
export function EndorsementTable({
  ...
  const data = items.map(
    isEntity ? mapEntityToEndorsement : mapSectionToEndorsement,
  );
  return (
    <div className="overflow-x-auto rounded-md border border-zinc-200 dark:border-zinc-800">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
          {data.map((row, i) => {
            const rawItem = items[i];
            const itemId =
              rawItem?.canonical_id || rawItem?.id || `endorsement-${i}`;
            return (
              <TooltipProvider key={i}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <tr
                      ...
                      onClick={() =>
                        onItemClick?.(
                          "endorsement",
                          row.canonicalId || rawItem?.id || `endorsement-${i}`,
                        )
                      }
                    >
                      <td className="px-3 py-3 align-top w-[220px] min-w-[180px]">
                      ...
                      <td className="px-3 py-3 align-top text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
                        {row.whatChanged ||
                          (row.impactedCoverage
                            ? `Modifies ${row.impactedCoverage}`
                            : "Endorsement modifying policy terms.")}
                      </td>
                    </tr>
                  </TooltipTrigger>
                  <TooltipContent side="left" className="text-[10px] py-1 px-2">
    ...
    <TooltipProvider>
      <div className="space-y-3">
        {data.map((row, i) => {
          const rawItem = items[i];
          const itemId =
            rawItem?.canonical_id || rawItem?.id || `modification-${i}`;
          return (
            <Tooltip key={i}>
              <TooltipTrigger asChild>
                <div
                  className="rounded-md border border-zinc-200 dark:border-zinc-800 overflow-hidden bg-white dark:bg-zinc-950 cursor-pointer group hover:border-[#1D3DCE]/30 transition-colors"
                ...
                >
                  ...
                  <div className="px-3 py-2.5 bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-2 flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                      <FileText className="size-3 text-zinc-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                      {row.severity && (
                        <Badge
                          variant="outline"
                          className={cn(
...
export function EntityTable({
  ...
  const entityId =
    (fields.id as string) ||
    (fields.canonical_id as string) ||
    `entity-${_index}`;

  // Map entity type to SourceType if possible
  const sourceTypeMap: Record<string, string> = {
    coverage: "effective_coverage",
    coverages: "effective_coverage",
    exclusion: "effective_exclusion",
    exclusions: "effective_exclusion",
    condition: "condition",
  ...
  const citationSourceType =
    sourceTypeMap[entity.entity_type.toLowerCase()] || "clause";
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div
            className="border border-zinc-200 dark:border-zinc-800 rounded overflow-hidden bg-white dark:bg-zinc-950 group hover:border-[#1D3DCE]/30 transition-colors cursor-pointer"
            onClick={() => onItemClick?.(citationSourceType, entityId)}
          >
