...
import { normalizeFieldLabel, formatValue, getSeverityBadgeStyle } from "@/utils/extraction-utils";
...
export function MinimalTableHeader({ headers }: { headers: string[] }) {
  return (
    <thead className="bg-zinc-50/50 dark:bg-zinc-900/50">
      <tr>
        {headers.map((header) => (
          <th key={header} className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 border-b border-zinc-100 dark:border-zinc-800">
            {header}
          </th>
        ))}
      </tr>
 ...
 */
export function SectionItemCard({ item, index }: { item: SectionItem; index: number }) {
