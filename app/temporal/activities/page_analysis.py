...
async def extract_page_signals_from_markdown(
    ...
    This activity avoids redundant PDF processing by using the markdown already
    generated by the OCR workflow.
    
    Args:
        document_id: UUID string of the document
        markdown_pages: List of (markdown_content, page_number, metadata) tuples
        
    Returns:
        List of page signal dictionaries
    """
    activity.logger.info(
        "[Phase 0: Page Analysis] Starting signal extraction from markdown",
        extra={
            "document_id": document_id,
            "activity": "extract_page_signals_from_markdown",
    ...
    try:
        async with async_session_maker() as session:
            ...
            # Extract signals using markdown analyzer
            signals, doc_type, confidence = await pipeline.extract_signals_from_markdown(
                document_id=UUID(document_id), 
                pages=markdown_pages
            )
            ...
            activity.logger.info(
                "Extracted signals from markdown",
                extra={
                    "document_id": document_id,
                    "signals_count": len(signals),
        ...
        activity.logger.error(
            "Failed to extract signals from markdown",
            extra={
                "document_id": document_id,
                "error": str(e)
            },
